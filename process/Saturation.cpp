...
Mat Saturation(const Mat &input, float set_sar){
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            result.at<Vec3b>(y, x) = Vec3b(\
                static_cast<uchar>(clamp(round((res_blue + mem) * 255), 0.0f, 255.0f)),\
                static_cast<uchar>(clamp(round((res_green + mem) * 255), 0.0f, 255.0f)),\
                static_cast<uchar>(clamp(round((res_red + mem) * 255), 0.0f, 255.0f))\
            );
        }
    }
